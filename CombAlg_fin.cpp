...
int main(int argc, char **argv){
    if (argc > 1 && (std::string(argv[1]) == "rows" || std::string(argv[1]) == "cols")) {
        std::ofstream out("work_time_" + std::string(argv[1]) + "_fin");
        ...
        for (int i = 0; i < 40; i++) {
	        ...
	        std::vector<double> w1;
                ...
                int n_iter = 0;
                int old_size = 0;
                while (X0.size() > 0 && X1.size() > 0 && X0.size() + X1.size() - old_size != 0 && n_iter < 8) { 
                    n_iter++;
                    std::cout << "ITER:" << n_iter << std::endl;
                    old_size = X0.size() + X1.size();
	            ...
	            std::vector<std::string> pred_X0 = predict(cl0, cl1, w0, w1, X0, y, "strict");
	            ...
	            X0 = new_X0;
	            std::vector<std::string> pred_X1 = predict(cl0, cl1, w0, w1, X1, y, "strict");
        ...
        std::ofstream out(std::string(argv[1]) + "_time_fin");
        std::ofstream errors("errors");
        std::vector<std::vector<short>> X0;
        std::vector<std::vector<short>> X1;
	...
	std::vector<double> w1;
	int k = 0;
	std::ofstream cl_x0(std::string(argv[1]) + "_" + y.first);
	std::ofstream cl_x1(std::string(argv[1]) + "_" + y.second);
	while (X0.size() > 0 && X1.size() > 0 && k < 20) {
            k++;
            errors << X0.size() << '*' << X0[0].size() << ' ' << X1.size() << '*' << X1[0].size() << std::endl;
            if (argc > 2 && std::string(argv[2]) == "show") {
                std::pair<std::vector<std::vector<short>>, int> result =  BinCombAlg(X0, X1, 1);
		cl0.insert(cl0.end(), result.first.begin(), result.first.end());
		cl_x0 << result.second << '\n';
		...
		cl_x1 << result.second << '\n';
                ...
                std::pair<std::vector<std::vector<short>>, int> result =  BinCombAlg(X0, old_X1, -100);
                cl0.insert(cl0.end(), result.first.begin(), result.first.end());
		cl_x0 << result.second << '\n';
                for (unsigned l = 0; l < result.first.size(); l++) {
                    for (unsigned ll = 0; ll < result.first[0].size(); ll++) {
	...
	std::cout << "FIT TIME: " << work_time << std::endl;
        std::ofstream res(std::string(argv[1]) + "_result_fin");
	...
	std::cout << "PREDICT TIME: " << work_time << std::endl;
        std::ofstream pred_str(std::string(argv[3]) + "_predict_fin");
        if (pred_str.is_open()) {
	...
	std::cout << "k iterations: " << k << std::endl;
        std::ofstream out_acc(std::string(argv[1]) + "_predict_acc_fin");
        out_acc << acc << '\n' << acc1 << std::endl;
        out_acc.close();
        errors.close();
    }
